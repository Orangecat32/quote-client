    ...
    tooltip: {
      ...
      formatter: function(params) {
        if (params && params.data && params.data[5]) {
          ...
          return `${firm.symbol}    MktCap: ${fmtMktCap(firm.mktCap)}B </br> ${
            firm.company
          }`;
        } else {
...
const buildSeries = (
  ...
  if (
    isNullOrWhitespace(selectedSector) &&
    isNullOrWhitespace(selectedSubIndustry)
  ) {
  ...
  if (isNullOrWhitespace(selectedSubIndustry)) {
    ...
    return subs.map((sub) =>
      fillSeries(sub, items.filter((f) => f[5].subIndustry === sub))
    );
  }

...
export const computeSubIndustryWeights = (portfolio, sector, subIndustries) => {
  ...
  (portfolio || [])
    ...
    .forEach((t) => (subs[t.subIndustry].value += t.mktCap));
...
export const sectorSubIndustries = (portfolio, sector) => {
  const obj = (portfolio || []).reduce((acc, ticker) => {
    return acc[ticker.subIndustry] || ticker.sector !== sector
      ? acc
      : { ...acc, [ticker.subIndustry]: true };
  }, {});
...
export const subIndustryFirms = (portfolio, subIndustry) => {
  const obj = (portfolio || []).reduce((acc, ticker) => {
    return ticker.subIndustry !== subIndustry
      ? acc
      : acc.concat({ value: ticker.mktCap, name: ticker.symbol });
  }, []);

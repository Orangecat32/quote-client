...
const enrichData = (t) => ({
  ...t,
  search: `${t.symbol} ${t.company} ${t.Location} ${t.subIndustry} ${
    t.sector
  }`.toLowerCase(),
...
const filterTicker = (ticker, f) => {
  ...
  return (
    (isNullOrWhitespace(search) ? true : ticker.search.includes(search)) &&
    (isNullOrWhitespace(f.selectedSector)
      ? true
      : ticker.sector === f.selectedSector)
  );
};
...
function mergeUpdate(portfolio, updates) {
  ...
  const keyedUpdates = updates.reduce(
    (acc, u) => Object.assign(acc, { [u.symbol]: u }),
    {}
  );
  ...
  return portfolio.map((item) => {
    ...
    return u
      ? Object.assign(item, { bid: u.bid, ask: u.ask, last: u.last, pc: u.pc })
      : item;
  });
}
...
export const allTickers = createSelector(
  [getTickers],
  (items) => {
...
export const sectorNames = createSelector(
  [getPortfolio],
  (portfolio) => {
...
export const allSectors = createSelector(
  [getPortfolio, sectorNames],
  (portfolio, sectorNames) => {
    return computeSectorWeights(sectorNames, portfolio);
  }
);

export const filteredTickers = createSelector(
  [allTickers, getFilters],
